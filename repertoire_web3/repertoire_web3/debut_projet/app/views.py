...
@app.route('/')
def main( nom = None ):
   return render_template( 'nouveau-Mail.html' )
   ...
   if request.method == 'POST':
      try:
         ...
         with sqlite3.connect("./repertoire.db") as db:
            cur = db.cursor()
            cur.execute(
               "INSERT INTO personne (prenom, nom, email, adresse, ville,codepostal) VALUES (?,?,?,?,?,?)",
               (prenom, nom, email, adresse, ville, codepostal)
               )
            db.commit()
...
@app.route('/recherche')
def new_recherche():
   return render_template('recherche-Mail.html')
